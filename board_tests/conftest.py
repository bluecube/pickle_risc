...
def pytest_addoption(parser):
    parser.addoption("--hw", action="store", default=None, help="TTY port for board tester (e.g. /dev/ttyUSB0). If not set, the tests run against the verilog models")
...
def board(request):
    ...
    if hw_port is None:
        ...
        verilator.clock = None # If clock is None, then auto-tracing is based on eval instead of clock
